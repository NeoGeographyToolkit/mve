...
void CameraInfo::read_tsai(std::string const& filename) {
  ...
  if (line.find("VERSION") != std::string::npos) {
    ...
    if (file_version == 4) {
      if (line.find("PINHOLE") == std::string::npos)
        throw std::invalid_argument(
          "CameraInfo::read_tsai: Expected PINHOLE type, got: " + line);
      std::getline(cam_file, line);
    }
  } else {
    ...
    if (std::sscanf(line.c_str(), "fu = %f", &dummy) != 1)
      throw std::invalid_argument(
        "CameraInfo::read_tsai: File must start with VERSION or fu = . "
        "Got: " + line);
  }
  ...
  if (std::sscanf(line.c_str(), "fu = %lf", &f_u) != 1)
    throw std::invalid_argument(
      "CameraInfo::read_tsai: Could not read x focal length.");

  std::getline(cam_file, line);
  if (std::sscanf(line.c_str(), "fv = %lf", &f_v) != 1)
    throw std::invalid_argument(
      "CameraInfo::read_tsai: Could not read y focal length.");
  ...
  if (std::sscanf(line.c_str(), "cu = %lf", &cu) != 1)
    throw std::invalid_argument(
      "CameraInfo::read_tsai: Could not read x principal point.");
  std::getline(cam_file, line);
  if (std::sscanf(line.c_str(), "cv = %lf", &cv) != 1)
    throw std::invalid_argument(
      "CameraInfo::read_tsai: Could not read y principal point.");
  ...
  if (std::sscanf(line.c_str(), "u_direction = %lf %lf %lf",
    ...
    throw std::invalid_argument(
      "CameraInfo::read_tsai: Could not read u direction.");
  std::getline(cam_file, line);
  if (std::sscanf(line.c_str(), "v_direction = %lf %lf %lf",
      &dummy3[0], &dummy3[1], &dummy3[2]) != 3)
    throw std::invalid_argument(
      "CameraInfo::read_tsai: Could not read v direction.");
  std::getline(cam_file, line);
  if (std::sscanf(line.c_str(), "w_direction = %lf %lf %lf",
      &dummy3[0], &dummy3[1], &dummy3[2]) != 3)
    throw std::invalid_argument(
      "CameraInfo::read_tsai: Could not read w direction.");
  ...
  if (std::sscanf(line.c_str(), "C = %lf %lf %lf",
    ...
    throw std::invalid_argument(
      "CameraInfo::read_tsai: Could not read camera center C.");
  ...
  if (std::sscanf(line.c_str(), "R = %lf %lf %lf %lf %lf %lf %lf %lf %lf",
    ...
    throw std::invalid_argument(
      "CameraInfo::read_tsai: Could not read rotation matrix R.");
